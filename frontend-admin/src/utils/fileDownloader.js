...
export function exportTransactionsToExcel(transactions = [], options = {}) {
    ...
    const userName = user.fullName || config.userName || 'Ban Quản trị SkillBridge';
    const userEmail = user.email || config.userEmail || 'admin@skillbridge.vn';
    const roleName = config.roleName || 'Quản trị viên hệ thống (Admin)';
    ...
    const bankInfo = bankAccount && bankAccount.accountNumber
        ...
        : 'Tài khoản thanh toán tổng hợp SkillBridge';
    const filterLabel = config.filterLabel || 'Báo cáo tổng hợp tài chính';
    const exportTime = new Date().toLocaleString('vi-VN');

    const cleanUser = (userName || 'Admin').replace(/[^a-zA-Z0-9_\u00C0-\u1EF9]/g, '_');
    ...
    const finalFileName = config.fileName
        ...
        : `Bao_cao_tai_chinh_SkillBridge_${cleanUser}_${nowTag}.xlsx`;
    ...
    const sheetData = [
        ...
        ['BÁO CÁO TÀI CHÍNH & SAO KÊ CHI TIẾT GIAO DỊCH'],
        ['(Chứng từ trích xuất chính thức từ hệ thống kế toán & quản trị SkillBridge)'],
        [],
        ['Đơn vị trích xuất:', userName, '', '', 'Thời điểm xuất:', exportTime, '', '', ''],
        ['Email quản trị:', userEmail, '', '', 'Vai trò tài khoản:', roleName, '', '', ''],
        ['Số dư khả dụng:', balance, '', '', 'Tiền ký quỹ (Escrow):', escrowLocked, '', '', ''],
        ['Tài khoản đối soát:', bankInfo, '', '', 'Kỳ / Bộ lọc báo cáo:', filterLabel, '', '', ''],
        ['Tổng số giao dịch:', `${transactions.length} giao dịch`, '', '', 'Trạng thái đối soát:', 'Hợp lệ — Đã đối chiếu toàn sàn', '', '', ''],
        [],
    ...
    transactions.forEach((t, idx) => {
        ...
        const txLabel = t.label || 'Giao dịch hệ thống';
    ...
    sheetData.push([]);
    sheetData.push(['* BÁO CÁO TÀI CHÍNH NỘI BỘ: Dữ liệu được tính toán và kiểm toán tự động từ cơ sở dữ liệu SkillBridge.']);
    ...
    const summaryRows = [
        ['BẢNG TỔNG HỢP DOANH THU & DÒNG TIỀN TOÀN HỆ THỐNG'],
        ['(Tổng hợp từ các giao dịch trong kỳ báo cáo tài chính)'],
        [],
    ...
    XLSX.utils.book_append_sheet(wb, ws2, 'Thống Kê Doanh Thu');
...
export function exportTransactionsToCSV(transactions = [], optionsOrFileName = 'Bao_cao_tai_chinh_SkillBridge.csv') {
    ...
    const fileName = config.fileName
        ...
        : `Bao_cao_tai_chinh_SkillBridge_${cleanUser}_${nowTag}.csv`;
    ...
    const lines = [
        ...
        [escapeCSV('BÁO CÁO TÀI CHÍNH & SAO KÊ CHI TIẾT GIAO DỊCH'), '""', '""', '""', '""', '""', '""', '""', '""'].join(','),
        [escapeCSV(`Thời gian trích xuất: ${exportTime}`), '""', '""', '""', '""', '""', '""', '""', '""'].join(','),
        [escapeCSV('Đơn vị trích xuất:'), escapeCSV(userName), '""', escapeCSV('Email:'), escapeCSV(userEmail), '""', escapeCSV('Kỳ báo cáo:'), escapeCSV(filterLabel), '""'].join(','),
